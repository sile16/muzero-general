...
class MuZeroConfig:
    def __init__(self):
        ...
        # - 28 board positions (including bar and bear-off)
        # - 5 additional flags
        # - 2 dice values
        # - 4 remaining dice
        # - 24 blots
        # - 24 blocks
        ### Game
        self.observation_shape = (1, 1, 87)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
        self.action_space = list(range(31))  # Fixed list of all possible actions. You should only edit the length
        ...
        self.fc_representation_layers = [128, 64 ]  # Define the hidden layers in the representation network
        ...
        self.train_on_gpu = torch.accelerator.is_available()  # Train on GPU if available
...
class Game(AbstractGame):
    ...
    def step(self, action):
        ...
        """
        observation, reward, done = self.bg_game.step(action)
        
        return observation, reward, done

    def to_play(self):
        return self.bg_game.player
    ...
    def reset(self):
        ...
        """
        self.bg_game.reset()
        self.bg_game.set_player(0)
        self.bg_game.roll_dice()

        return self.bg_game.get_observation()
